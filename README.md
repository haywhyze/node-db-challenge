...
1. Explain the difference between `Relational Databases` and `SQL`.
   Relational Database is a type of database where a collection of multiple data sets are organized by tables, records and columns. SQL which is an acronym for Structured Query Language is alanguage used to query databases such as a Relational Database.

2. Why do tables need a `primary key`?
   Tables need a primary key for unique identification of records.

3. What is the name given to a table column that references the primary key on another table.
   Foreign key

4. What do we need in order to have a _many to many_ relationship between two tables.
   An intermediary table to hold foreign keys that reference the primary key on the related tables.
